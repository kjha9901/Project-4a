...
let (toks_after, expr) = parse_base toks2 in 
(toks_after, Not expr)
  | _ -> parse_base toks
...
and parse_pow toks =
  ...
  | Tok_Pow -> 
let toks2 = match_token toks_after Tok_Pow in
let (toks3, expr_after) = parse_pow toks2 in
 (toks3, Pow (expr, expr_after))
  | _ -> (toks_after, expr)
...
and parse_mult toks =
  ...
  match (lookahead toks_after) with
  | Tok_Mult -> 
let toks2 = match_token toks_after Tok_Mult in
let (toks3, expr_after) = parse_mult toks2 in
(toks3, Mult (expr, expr_after))
  | Tok_Div -> 
let toks2 = match_token toks_after Tok_Div in
let (toks3, expr_after) = parse_mult toks2 in 
(toks3, Div (expr, expr_after))
  | _ -> (toks_after, expr)
...
and parse_greater_less toks =
  ...
  match (lookahead toks_after) with
  | Tok_Greater -> 
let toks2 = match_token toks_after Tok_Greater in
let (toks3, expr_after) = parse_greater_less toks2 in 
(toks3, Greater (expr, expr_after))
