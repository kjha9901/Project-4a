...
(*
let rec parse_Expressions toks =
  
  let a1 = parse_Or toks in
  let t = lookahead toks in
  match t with
  | Tok_If -> 
    match_token t Tok_If
    let a2 = parse_Expressions t in
...
let rec parse_Prime toks = 
  ...
  | Some Tok_LParen -> let tok2 = match_token toks (Tok_LParen) in
    ...
    let tok4 = match_token tok3 (Tok_RParen) in (tok4, expr)
...
and parse_FunctionCall toks =
  let (toksTail, expr) = parse_Prime toks in
...
(*
and parse_If toks =
  let (toksTail, expr) = parse_Or toks in
  match (lookahead toksTail) with
  | Some Tok_If ->
    let tok2 = match_token toksTail Tok_If in
    let (toksTailExpr1, expr1) = parse_If tok2 in
  | Some Tok_Then ->
    let tok3 = match_token toks Tok_Then in
    let (toksTailExpr2, expr2) = parse_If tok3 in
  | Some Tok_Else ->    
    let tok4 = match_token toks Tok_Else in
    let (toksTailExpr3, expr3) = parse_If tok4 in
    
    (*let (toksTail, expr) = parse_If toksTailExpr3 in*)
    (toksTailExpr3, If(expr1, expr2, expr3))
    
  | _ -> (toksTail, expr)

and parse_Fun toks =
  let (toksTail, expr) = parse_If toks in
  match (lookahead toksTail) with
  | Some Tok_Fun ->
    let tok2 = match_token toksTail Tok_Fun in
    (match (lookahead tok2) with
    | Some Tok_ID(id) ->
      let tok3 = match_token tok2 (Tok_ID(id)) in
      let tok4 = match_token tok3 Tok_Arrow in
      let (tok5, exprTail) = parse_Fun tok4 in
      (tok5, Fun(id, exprTail))
    | _ -> raise (InvalidInputException("Invalid Func input")))
  | _ -> (toksTail, expr)

and parse_Let toks =
  let (toksTail, expr) = parse_Fun toks in
  match (lookahead toksTail) with
  | Some Tok_Let ->
    let tok2 = match_token toksTail Tok_Let in
    if (lookahead tok2) = Some Tok_Rec then
      ...
      | Some Tok_ID(id) ->
        ...
        let (toksTailExpr, expr) = parse_Let tok5 in
        let tok6 = match_token toksTailExpr Tok_In in
        let (toksTailExpr2, expr2) = parse_Let tok6 in
        (toksTailExpr2, Let(id, true, expr, expr2))
      ...
      | Some Tok_ID(id) ->
        ...
        let (toksTailExpr, expr) = parse_Let tok4 in
        let tok5 = match_token toksTailExpr Tok_In in
        let (toksTailExpr2, expr2) = parse_Let tok5 in
        (toksTailExpr2, Let(id, false, expr, expr2))
      | _ -> raise (InvalidInputException("Invalid Let input")))
  | _ -> (toksTail, expr)
*)
;;
